...
| ADR-7 | **帧内含布局坐标**（树/图 x,y ∈ [0,1] 相对坐标） | 快照自包含，渲染器零布局逻辑；图拖拽通过"实时坐标覆盖表"解决（见下） |
...
帧内的节点坐标是运行那一刻模型坐标的拷贝。用户拖动节点时：
- 编辑器把新坐标写入 `覆盖表 livePositions: Map<id,{x,y}>`（React 状态）；
- GraphView 渲染时优先取覆盖表坐标；
- 用户再次点击"运行"后，新帧自带新坐标，覆盖表清空。
