...
export function* linearSearchGen(input: SearchInput): Generator<VizStep, void, void> {
  ...
  for (let i = 0; i < arr.length; i++) {
    c.comparisons++;
    yield emit(`检查 a[${i}]=${arr[i]} 是否等于目标值 ${target}`, [1, 2], { comparing: [i] });
    if (arr[i] === target) {
      yield emit(`a[${i}]=${target} 与目标值相等，查找成功！共比较 ${c.comparisons} 次`, [3], {
        found: i,
      });
      return;
    }
...
export function* binarySearchGen(input: SearchInput): Generator<VizStep, void, void> {
  ...
  if (n === 0) {
    yield emit(`数组为空，目标值 ${target} 不存在，查找失败`, [10]);
    return;
  }
  let lo = 0;
  let hi = n - 1;
  while (lo <= hi) {
    const mid = (lo + hi) >> 1;
    c.comparisons++;
    yield emit(`当前搜索区间 [${lo}..${hi}]，取中点 mid=${mid}（a[${mid}]=${arr[mid]}）`, [2, 3], {
      range: [lo, hi],
      pointers: { lo, hi, mid },
      comparing: [mid],
    });
    if (arr[mid] === target) {
      yield emit(`a[${mid}]=${target} 正是目标值，查找成功！共比较 ${c.comparisons} 次`, [4, 5], {
        range: [lo, hi],
        found: mid,
        pointers: { lo, hi, mid },
      });
      ...
      lo = mid + 1;
      yield emit(`a[${mid}]=${arr[mid]} < ${target}：目标只可能在右半，lo ← ${lo}`, [6, 7], {
        range: [lo, hi],
        pointers: { lo, hi },
      });
    } else {
      hi = mid - 1;
      yield emit(`a[${mid}]=${arr[mid]} > ${target}：目标只可能在左半，hi ← ${hi}`, [8, 9], {
        range: [lo, hi],
        pointers: { lo, hi },
      });
    }
