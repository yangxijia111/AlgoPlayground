    ...
    sections: [
      {
        heading: '它是什么（What）',
        paragraphs: [
          '算法（algorithm）是为解决某个问题而设计的、一步一步的有限操作序列。它必须是明确的：每一步做什么、下一步从哪里开始，都不能含糊（确定性）。',
    ...
    sections: [
      {
        ...
        paragraphs: [
          ...
          '[[term:node|节点]]和边组成更复杂的树与图之前，数组和链表是最基础的两类：数组把元素放在连续的位置上（可以用下标直接定位），链表把元素用「指针」串起来（只能顺藤摸瓜）。',
        ],
      },
      {
        heading: '为什么重要（Why）',
        paragraphs: [
          ...
          '算法和数据结构密不可分：二分查找依赖「有序数组」，广度优先搜索依赖「[[term:queue|队列]]」。选对了结构，算法才成立。',
        ],
      },
      {
        heading: '关键要点（Key Points）',
    ...
    sections: [
      {
        heading: '它是什么（What）',
        paragraphs: [
          '[[term:time-complexity]]（time complexity）描述算法的执行步数随输入规模 n 增长的大致趋势，常用大 O 记号表示，如 O(n)、O(n²)。',
      ...
      },
      {
        ...
        bullets: [
          ...
          '每次把问题砍半的步骤贡献 log 因子；',
          '[[term:time-complexity|大 O 记号]]只保留最高阶项、忽略常数：3n² + 5n + 2 记作 O(n²)。',
        ],
      },
    ],
    ...
    sections: [
      {
        heading: '它是什么（What）',
        paragraphs: [
          '[[term:space-complexity]]（space complexity）描述算法运行时所需的额外存储空间随输入规模 n 的增长趋势，同样用大 O 记号。',
          '例如冒泡排序只多用了几个变量（O(1)，称「[[term:in-place|原地排序]]」）；归并排序需要与原数组等大的辅助数组（O(n)）；[[term:recursion|递归]]调用本身会占用[[term:call-stack|调用栈]]——深度为 n 的递归就是 O(n) 的栈空间。',
        ],
      },
      {
        heading: '为什么重要（Why）',
